...
import { Pencil, Check, X, ChevronDown, MessageSquare, Plus, Search, Loader2 } from 'lucide-react';
import { CATEGORY_COLORS } from '@/lib/categories';
import { addCustomRule, getCustomCategories, addCustomCategory, countMatchingTransactions } from '@/lib/storage';
...
type EditorState = 'picking' | 'confirming' | 'saving' | 'done';
...
export function CategoryEditor({ transaction, onSaved }: CategoryEditorProps) {
  ...
  // Confirmation state
  const [editorState, setEditorState] = useState<EditorState>('picking');
  const [matchCount, setMatchCount] = useState<{ total: number; otherCategory: number } | null>(null);
  ...
  // Derive the pattern that will be used for matching
  ...
  const handleSelectCategory = (cat: string) => {
    ...
    setCreatingNew(false);
    // Reset confirmation state when category changes
    setEditorState('picking');
    setMatchCount(null);
  };
  ...
  const handleSave = async () => {
    ...
    // Count matches first to show confirmation
    setEditorState('confirming');
    const counts = await countMatchingTransactions(pattern);
    setMatchCount(counts);

    // If only 1 match (this transaction), skip confirmation and apply immediately
    if (counts.total <= 1) {
      await applyRule();
    }
  };
  ...
  const applyRule = async () => {
    ...
    const count = await addCustomRule({
      pattern,
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 mt-1 w-72 bg-card border border-card-border rounded-xl shadow-2xl overflow-hidden">
          {/* Confirmation state — asking user to confirm batch apply */}
          {editorState === 'confirming' && matchCount && matchCount.total > 1 && (
            <div className="p-4 space-y-3">
              <div className="flex items-start gap-2">
                <div className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: allColors[selectedCategory] || '#a1a1aa' }} />
                <div>
                  <p className="text-sm font-medium text-foreground">
                    Apply &ldquo;{selectedCategory}&rdquo; to all?
                  </p>
                  <p className="text-xs text-muted mt-1">
                    Found <span className="text-foreground font-medium">{matchCount.total} transactions</span> matching
                    <span className="text-foreground font-mono text-[11px]"> &ldquo;{pattern.length > 25 ? pattern.slice(0, 25) + '...' : pattern}&rdquo;</span>
                  </p>
                  {matchCount.otherCategory > 0 && (
                    <p className="text-xs text-amber-400 mt-0.5">
                      {matchCount.otherCategory} of these are uncategorized
                    </p>
                  )}
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={applyRule}
                  className="flex-1 flex items-center justify-center gap-1.5 bg-accent hover:bg-accent-hover text-white text-xs font-medium px-3 py-2 rounded-md transition-colors"
                >
                  <Check className="h-3 w-3" />
                  Apply to all {matchCount.total}
                </button>
                <button
                  onClick={() => {
                    setEditorState('picking');
                  ...
                  className="flex items-center justify-center text-xs text-muted hover:text-foreground px-3 py-2 rounded-md border border-card-border hover:bg-card-border/40 transition-colors"
                >
                  Back
                </button>
              </div>
          ...
          {editorState === 'saving' && (
            <div className="p-6 flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-muted">Applying rule...</p>
            </div>
          )}
          ...
          {editorState === 'done' && (
            <div className="p-5 flex flex-col items-center gap-2">
              ...
              <p className="text-sm font-medium text-foreground">
                {appliedCount} transaction{appliedCount !== 1 ? 's' : ''} updated
              </p>
              <p className="text-xs text-muted">
                All &ldquo;{pattern.length > 20 ? pattern.slice(0, 20) + '...' : pattern}&rdquo; now categorized as {selectedCategory}
              </p>
            </div>
          )}
              ...
              {selectedCategory && (
                <div className="border-t border-card-border p-3 space-y-3">
                  ...
                  <p className="text-[10px] text-muted/70">
                    Rule pattern: <span className="font-mono text-foreground/50">{pattern.length > 30 ? pattern.slice(0, 30) + '...' : pattern}</span>
                  </p>
                </div>
              )}
            </>
